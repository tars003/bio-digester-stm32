...
void setup() {
  ...
  // INIT GPRS
  gsm_config_gprs();
  lcd.setCursor(0, 0); lcd.print("GPRS configuration");
...
void gsm_http_post( String postdata) {
  ...
  if(!isReqSuccess) {
    lcd.clear();
    lcd.setCursor(0, 0); lcd.print("Send data failed !!!");
    lcd.setCursor(0, 1); lcd.print("Fail count : "); lcd.print(reqFailCount);  
    ledSignal(10);
  }
  else {
    ledSignal(2);  
    lcd.clear();
    lcd.setCursor(0, 0); lcd.print("Send data DONE !");    
  }
  delay(lcdInfoDelay);
}
